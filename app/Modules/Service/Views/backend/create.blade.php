...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Buat Service</h1>
            <p class="text-gray-600 mt-1">Tambahkan service baru</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
        <form action="{{ route('services.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            {{-- Name Field --}}
            {!! formField('layanan_nama', 'text', 'Nama', null, [], [
                'required' => true,
                'placeholder' => 'Masukkan nama service',
                'help' => 'Nama service akan ditampilkan di halaman layanan'
            ]) !!}

            {{-- Description Field (Textarea) --}}
            {!! formField('layanan_deskripsi', 'textarea', 'Deskripsi', null, [], [
                'required' => true,
                'rows' => 5,
                'placeholder' => 'Masukkan deskripsi lengkap service',
                'help' => 'Deskripsi akan ditampilkan di halaman detail service'
            ]) !!}

            {{-- Image Field --}}
            {!! formField('layanan_gambar', 'file', 'Gambar', null, [], [
                'accept' => 'image/*',
                'help' => 'Format: JPG, PNG (Max 2MB)'
            ]) !!}

            {{-- Order Field --}}
            {!! formField('layanan_urutan', 'number', 'Urutan', null, [], [
                'min' => 0,
                'placeholder' => '0',
                'help' => 'Urutan tampilan service'
            ]) !!}

            {{-- Status Field --}}
            {!! formField('layanan_status', 'select', 'Status', null, [
                '1' => 'Aktif',
                '0' => 'Nonaktif'
            ], [
                'required' => true,
