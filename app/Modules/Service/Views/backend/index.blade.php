...
<div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Layanan</h1>
            <p class="text-gray-500 text-sm mt-0.5">Kelola semua layanan perusahaan</p>
        </div>
            ...
            <table class="w-full min-w-[480px]">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $service->layanan_nama }}</td>
                        <td class="px-4 py-3 text-sm text-gray-600 hidden sm:table-cell">{{ Str::limit(strip_tags($service->layanan_deskripsi), 50) }}</td>
                        <td class="px-4 py-3 hidden sm:table-cell">
                            @if($service->layanan_gambar)
                                <img src="{{ asset('storage/' . $service->layanan_gambar) }}" alt="{{ $service->layanan_nama }}" class="h-10 w-10 object-cover rounded-lg">
                            @else
                        ...
                        <td class="px-4 py-3">
                            <span class="text-xs font-semibold px-2.5 py-1 rounded-full whitespace-nowrap
                                {{ $service->layanan_status ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600' }}">
                                {{ $service->layanan_status ? 'Aktif' : 'Nonaktif' }}
                            </span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex gap-3 items-center">
                                <a href="{{ route('services.edit', $service) }}" class="text-blue-500 hover:text-blue-800 p-1">
                    ...
                    <tr>
                        <td colspan="5" class="px-4 py-10 text-center text-sm text-gray-500">
                            Belum ada layanan. <a href="{{ route('services.create') }}" class="text-teal-600 font-semibold">Buat yang pertama</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
