...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 mt-1">{{ $faq->faq_pertanyaan }}</p>
        </div>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        <form action="{{ route('faqs.update', $faq) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')

            {{-- Question Field --}}
            {!! formField('faq_pertanyaan', 'text', 'Pertanyaan', $faq->faq_pertanyaan, [], [
                'required' => true,
                'placeholder' => 'Masukkan Pertanyaan'
            ]) !!}

            {{-- Answer Field --}}
            {!! formField('faq_jawaban', 'text', 'Jawaban', $faq->faq_jawaban, [], [
                'required' => true,
            ...
            {{-- Order Field --}}
            {!! formField('faq_urutan', 'number', 'Urutan', $faq->faq_urutan, [], [
                'min' => 0,
                'placeholder' => '0',
            ...
            {{-- Status Field --}}
            {!! formField('faq_status', 'select', 'Status', $faq->faq_status, [
                '1' => 'Active',
                '0' => 'Inactive'
            ], [
                'required' => true,
