...
<div class="space-y-6">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-2xl font-bold text-gray-900">Tambah Header</h1>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('headers.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Section <span class="text-red-500">*</span></label>
                <input type="text" name="header_section" value="{{ old('header_section') }}" required
                       placeholder="contoh: about, products, services"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 focus:outline-none @error('header_section') border-red-400 @enderror">
                <p class="text-xs text-gray-400 mt-1">Key unik untuk mengidentifikasi section (huruf kecil, tanpa spasi)</p>
                @error('header_section') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="header_subtitle" rows="4"
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 focus:outline-none">{{ old('header_subtitle') }}</textarea>
            </div>
