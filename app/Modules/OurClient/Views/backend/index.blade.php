...
<div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Mitra</h1>
            <p class="text-gray-500 text-sm mt-0.5">Kelola semua mitra / klien perusahaan</p>
        </div>
    ...
    <!-- Search -->
    <form method="GET" action="{{ route('ourclient.index') }}" class="flex gap-2">
            ...
            <table class="w-full min-w-[480px]">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wide hidden sm:table-cell">Logo</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $client->klien_nama }}</td>
                        <td class="px-4 py-3 hidden sm:table-cell">
                            @if($client->klien_gambar)
                                <img src="{{ asset('storage/' . $client->klien_gambar) }}" alt="{{ $client->klien_nama }}" class="h-10 w-auto object-contain rounded">
                            @else
                        ...
                        <td class="px-4 py-3">
                            <span class="text-xs font-semibold px-2.5 py-1 rounded-full whitespace-nowrap
                                {{ $client->klien_status ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600' }}">
                                {{ $client->klien_status ? 'Aktif' : 'Nonaktif' }}
                            </span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex gap-3 items-center">
                                <a href="{{ route('ourclient.edit', $client) }}" class="text-blue-500 hover:text-blue-800 p-1">
                                    <i data-lucide="pencil" class="w-4 h-4"></i>
                                </a>
                                <form action="{{ route('ourclient.destroy', $client) }}" method="POST" class="inline"
                    ...
                    <tr>
                        <td colspan="4" class="px-4 py-10 text-center text-sm text-gray-500">
                            Belum ada mitra. <a href="{{ route('ourclient.create') }}" class="text-teal-600 font-semibold">Buat yang pertama</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
