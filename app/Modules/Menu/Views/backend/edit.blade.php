...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 text-sm mt-0.5">{{ $menu->menu_nama }}</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
        <form action="{{ route('menus.update', $menu) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="menu_nama" value="{{ old('menu_nama', $menu->menu_nama) }}"
                    ...
                    <select name="menu_kategori_id"
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->kategori_id }}"
                                {{ old('menu_kategori_id', $menu->menu_kategori_id) == $cat->kategori_id ? 'selected' : '' }}>
                                {{ $cat->kategori_nama }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="menu_harga" value="{{ old('menu_harga', $menu->menu_harga) }}"
                ...
                <div>
                    ...
                    <select name="menu_status"
                        ...
                        <option value="aktif" {{ old('menu_status', $menu->menu_status) === 'aktif' ? 'selected' : '' }}>Aktif</option>
                        <option value="tidak aktif" {{ old('menu_status', $menu->menu_status) === 'tidak aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                    </select>
            ...
            <div>
                ...
                <textarea name="menu_deskripsi" rows="4"
                          ...
                          class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 @error('menu_deskripsi') border-red-500 @enderror">{{ old('menu_deskripsi', $menu->menu_deskripsi) }}</textarea>
